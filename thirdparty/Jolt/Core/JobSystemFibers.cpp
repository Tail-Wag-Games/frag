...
#include <Jolt/Core/JobSystemFibers.h>
...
void JobSystemFibers::Init(uint inMaxJobs, uint inMaxBarriers, int inNumThreads)
{
...
JobSystemFibers::JobSystemFibers(uint inMaxJobs, uint inMaxBarriers, int inNumThreads)
{
	Init(inMaxJobs, inMaxBarriers, inNumThreads);
}

void JobSystemFibers::StartThreads(int inNumThreads)
{
...
JobSystemFibers::~JobSystemFibers()
{
	// Stop all worker threads
	StopThreads();
}

void JobSystemFibers::StopThreads()
{
	if (mThreads.empty())
		return;
...
JobHandle JobSystemFibers::CreateJob(const char *inJobName, ColorArg inColor, const JobFunction &inJobFunction, uint32 inNumDependencies)
{
...
void JobSystemFibers::FreeJob(Job *inJob)
{
	mJobs.DestructObject(inJob);
}

uint JobSystemFibers::GetHead() const
{
...
void JobSystemFibers::QueueJobInternal(Job *inJob)
{
...
void JobSystemFibers::QueueJob(Job *inJob)
{
	JPH_PROFILE_FUNCTION();
...
void JobSystemFibers::QueueJobs(Job **inJobs, uint inNumJobs)
{
	JPH_PROFILE_FUNCTION();
...
void JobSystemFibers::ThreadMain(int inThreadIndex)
{
	// Name the thread
	char name[64];
