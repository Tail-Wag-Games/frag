...
float* ozz_joint_upload_buffer(void)
{
    assert(state.valid);
    return state.joint_upload_buffer;
}

int ozz_joint_texture_pitch(void)
{
    assert(state.valid);
    return state.joint_texture_pitch;
}

int ozz_joint_texture_width(void)
{
    assert(state.valid);
    return state.joint_texture_width;
}

int ozz_joint_texture_height(void)
{
    assert(state.valid);
    return state.joint_texture_height;
}
...
void ozz_load_mesh(ozz_instance_t *ozz, const void *data, size_t num_bytes, create_vbuf_callback_t create_vbuf_cb, create_ibuf_callback_t create_ibuf_cb)
{
    assert(state.valid && ozz && data && (num_bytes > 0));
    ozz_private_t *self = (ozz_private_t *)ozz;
    ozz::io::MemoryStream stream;
    ...
    if (archive.TestTag<ozz::sample::Mesh>())
    {
        ...
        create_vbuf_cb(vertices, num_vertices);
        free(vertices); vertices = nullptr;

        create_ibuf_cb(&mesh.triangle_indices[0], self->num_triangle_indices);
    }
    else
    {
        self->load_failed = true;
