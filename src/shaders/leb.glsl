...
leb_CreateSameDepthNeighborIDs(uint left, uint right, uint edge, uint node)
{
...
leb_DiamondParent
leb_CreateDiamondParent(in const cbt_Node base, in const cbt_Node top)
{
 ...
 *
 */
uint leb_GetBitValue(const uint bitField, int bitID)
{
 ...
 */
leb_SameDepthNeighborIDs
leb_SplitNodeIDs(in const leb_SameDepthNeighborIDs nodeIDs, uint splitBit)
{
    ...
    return leb_CreateSameDepthNeighborIDs(
...
leb_DecodeSameDepthNeighborIDs(in const cbt_Node node)
{
    leb_SameDepthNeighborIDs nodeIDs =
        leb_CreateSameDepthNeighborIDs(0u, 0u, 0u, 1u);

    for (int bitID = node.depth - 1; bitID >= 0; --bitID) {
        nodeIDs = leb_SplitNodeIDs(nodeIDs, leb_GetBitValue(node.id, bitID));
    }
...
leb_DecodeSameDepthNeighborIDs_Square(in const cbt_Node node)
{
    ...
    leb_SameDepthNeighborIDs nodeIDs =
        leb_CreateSameDepthNeighborIDs(0u, 0u, 3u - b, 2u + b);
 ...
 *
 */
cbt_Node leb_EdgeNeighbor(in const cbt_Node node)
{
...
cbt_Node leb_EdgeNeighbor_Square(in const cbt_Node node)
{
...
void leb_SplitNode(const int cbtID, in const cbt_Node node)
{
    if (!cbt_IsCeilNode(cbtID, node)) {
        ...
        nodeIterator = leb_EdgeNeighbor(nodeIterator);
...
leb_MergeNode(
    ...
    if (!cbt_IsRootNode(node) && leb_HasDiamondParent(cbtID, diamondParent)) {
...
leb_MergeNode_Square(
    ...
    if ((node.depth > 1) && leb_HasDiamondParent(cbtID, diamondParent)) {
        cbt_MergeNode(cbtID, node);
    }
}
 ...
 *
 */
mat3 leb_SplittingMatrix(uint splitBit)
{
 ...
 *
 */
mat3 leb_SquareMatrix(uint quadBit)
{
 ...
 *
 */
mat3 leb_WindingMatrix(uint mirrorBit)
{
 ...
 *
 */
mat3 leb_DecodeTransformationMatrix(in const cbt_Node node)
{
    mat3 xf = mat3(1.0f);

    for (int bitID = node.depth - 1; bitID >= 0; --bitID) {
        xf = leb_SplittingMatrix(leb_GetBitValue(node.id, bitID)) * xf;
    }

    return leb_WindingMatrix(node.depth & 1) * xf;
}

mat3 leb_DecodeTransformationMatrix_Square(in const cbt_Node node)
{
    int bitID = max(0, node.depth - 1);
    mat3 xf = leb_SquareMatrix(leb_GetBitValue(node.id, bitID));

    for (bitID = node.depth - 2; bitID >= 0; --bitID) {
        xf = leb_SplittingMatrix(leb_GetBitValue(node.id, bitID)) * xf;
    }

    return leb_WindingMatrix((node.depth ^ 1) & 1) * xf;
}
...
vec3 leb_DecodeNodeAttributeArray(in const cbt_Node node, in const vec3 data)
{
    return leb_DecodeTransformationMatrix(node) * data;
}
...
vec3 leb_DecodeNodeAttributeArray_Square(in const cbt_Node node, in const vec3 data)
{
    return leb_DecodeTransformationMatrix_Square(node) * data;
}
