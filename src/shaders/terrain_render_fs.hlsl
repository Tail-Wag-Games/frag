...
{
    float _75_u_TargetEdgeLength : packoffset(c0);
    float _75_u_LodFactor : packoffset(c0.y);
    float _75_u_DmapFactor : packoffset(c0.z);
    float _75_u_MinLodVariance : packoffset(c0.w);
};

ByteAddressBuffer u_CbtBuffers[1] : register(t1);
cbuffer PerFrameVariables : register(b0)
{
    row_major float4x4 _136_u_ModelMatrix : packoffset(c0);
    row_major float4x4 _136_u_ModelViewMatrix : packoffset(c4);
    row_major float4x4 _136_u_ViewMatrix : packoffset(c8);
    row_major float4x4 _136_u_CameraMatrix : packoffset(c12);
    row_major float4x4 _136_u_ViewProjectionMatrix : packoffset(c16);
    row_major float4x4 _136_u_ModelViewProjectionMatrix : packoffset(c20);
    float4 _136_u_FrustumPlanes[6] : packoffset(c24);
};
...
void frag_main()
{
    ...
    float sx = sx1 - sx0;
    float sy = sy1 - sy0;
    float3 n = normalize(float3(float2(-sx, -sy) * (((_75_u_DmapFactor * 0.02999999932944774627685546875f) / filterSize) * 0.5f), 1.0f));
    ...
    o_FragColor = float4(shading * 0.5f, 1.0f);
}
