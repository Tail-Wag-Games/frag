...
layout(std430, binding = CBT_HEAP_BUFFER_BINDING) buffer cbt_Buffer {
    uint heap[];
 ...
 */
void
cbt_SetBitValue(const int cbtID, uint bufferID, uint bitID, uint bitValue)
{
 ...
 *
 */
cbt_Node cbt_CeilNode_Fast(const int cbtID, in const cbt_Node node)
{
    int maxDepth = cbt_MaxDepth(cbtID);
    return cbt_CreateNode(node.id << (maxDepth - node.depth), maxDepth);
}

cbt_Node cbt_CeilNode(const int cbtID, in const cbt_Node node)
{
    return cbt_IsNullNode(node) ? node : cbt_CeilNode_Fast(cbtID, node);
}
 ...
 *
 */
uint cbt_NodeBitID(const int cbtID, in const cbt_Node node)
{
 ...
 */
uint cbt_NodeBitID_BitField(const int cbtID, in const cbt_Node node)
{
 ...
 *
 */
int cbt_NodeBitSize(const int cbtID, in const cbt_Node node)
{
...
cbt_HeapArgs
cbt_CreateHeapArgs(const int cbtID, in const cbt_Node node, int bitCount)
{
    uint alignedBitOffset = cbt_NodeBitID(cbtID, node);
    uint maxHeapIndex = cbt_HeapUint32Size(cbt_MaxDepth(cbtID)) - 1u;
 ...
 */
void
cbt_HeapWriteExplicit(
    const int cbtID,
    in const cbt_Node node,
    int bitCount,
    uint bitData
) {
    cbt_HeapArgs args = cbt_CreateHeapArgs(cbtID, node, bitCount);

    cbt_BitFieldInsert(cbtID,
                        args.heapIndexLSB,
                        args.bitOffsetLSB,
                        args.bitCountLSB,
                        bitData);
...
void cbt_HeapWrite(const int cbtID, in const cbt_Node node, uint bitData)
{
    cbt_HeapWriteExplicit(cbtID, node, cbt_NodeBitSize(cbtID, node), bitData);
}
 ...
 */
uint
cbt_HeapReadExplicit(const int cbtID, in const cbt_Node node, int bitCount)
{
    cbt_HeapArgs args = cbt_CreateHeapArgs(cbtID, node, bitCount);
    uint lsb = cbt_BitFieldExtract(u_CbtBuffers[cbtID].heap[args.heapIndexLSB],
                                    args.bitOffsetLSB,
                                    args.bitCountLSB);
    uint msb = cbt_BitFieldExtract(u_CbtBuffers[cbtID].heap[args.heapIndexMSB],
                                    0u,
                                    args.bitCountMSB);
...
uint cbt_HeapRead(const int cbtID, in const cbt_Node node)
{
    return cbt_HeapReadExplicit(cbtID, node, cbt_NodeBitSize(cbtID, node));
}
 ...
 */
void
cbt_HeapWrite_BitField(const int cbtID, in const cbt_Node node, uint bitValue)
{
    uint bitID = cbt_NodeBitID_BitField(cbtID, node);

    cbt_SetBitValue(cbtID, bitID >> 5u, bitID & 31u, bitValue);
}
 ...
 *
 */
uint cbt_HeapRead_BitField(const int cbtID, in const cbt_Node node)
{
    uint bitID = cbt_NodeBitID_BitField(cbtID, node);

    return cbt_GetBitValue(u_CbtBuffers[cbtID].heap[bitID >> 5u], bitID & 31u);
}
...
void cbt_SplitNode_Fast(const int cbtID, in const cbt_Node node)
{
    cbt_HeapWrite_BitField(cbtID, cbt_RightChildNode(node), 1u);
}
void cbt_SplitNode(const int cbtID, in const cbt_Node node)
{
...
void cbt_MergeNode_Fast(const int cbtID, in const cbt_Node node)
{
    cbt_HeapWrite_BitField(cbtID, cbt_RightSiblingNode(node), 0u);
}
void cbt_MergeNode(const int cbtID, in const cbt_Node node)
{
